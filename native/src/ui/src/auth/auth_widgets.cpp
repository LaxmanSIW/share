...
PasswordFieldWithToggle::PasswordFieldWithToggle(QWidget* parent) : QLineEdit(parent) {
  ...
  // (addAction returns a QAction; we don't need it — the button is shown.)
  { auto* a = new QWidgetAction(this); a->setDefaultWidget(toggle_btn_); addAction(a, QLineEdit::TrailingPosition); }
