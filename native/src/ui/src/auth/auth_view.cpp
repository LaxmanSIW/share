...
AuthView::AuthView(QWidget* parent) : QFrame(parent) {
  ...
  static_cast<QHBoxLayout*>(brand_row->layout())->addItem(brand_text);
  static_cast<QHBoxLayout*>(brand_row->layout())->addItem(UiTheme::hspacer());
...
void AuthView::render_state_(State s) {
  ...
  if (s == State::SignIn) {
    ...
    static_cast<QHBoxLayout*>(row->layout())->addItem(UiTheme::hspacer());
    ...
    static_cast<QHBoxLayout*>(row2->layout())->addItem(UiTheme::hspacer());
