...
fin::model::PurchaseBill purchase_bill_from_json(std::string_view s) {
  ...
  try {
    ...
    if (j.contains("total")) j.at("total").get_to(p.total); else p.total = fin::Money::zero(fin::CurrencyId::INR);
    if (j.contains("itc")) j.at("itc").get_to(p.itc); else p.itc = fin::Money::zero(fin::CurrencyId::INR);
    p.status = j.value("status", "draft");
    p.notes = j.value("notes", "");
    if (j.contains("paid")) j.at("paid").get_to(p.paid); else p.paid = fin::Money::zero(fin::CurrencyId::INR);
    if (j.contains("due")) j.at("due").get_to(p.due); else p.due = fin::Money::zero(fin::CurrencyId::INR);
...
fin::model::Transaction transaction_from_json(std::string_view s) {
  ...
  try {
    ...
    if (j.contains("amount")) j.at("amount").get_to(t.amount); else t.amount = fin::Money::zero(fin::CurrencyId::INR);
