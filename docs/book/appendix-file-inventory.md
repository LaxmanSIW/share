# Appendix A5 (living): File Inventory

> This is the **complete inventory** of every tracked file in the repository, with its
> role in the book. It doubles as the coverage ledger: by the end of the book, every row
> is claimed by exactly one chapter, and the final coverage audit (Appendix A5, final
> part) proves each file was explained in full.
>
> Legend: **Ch** = the chapter that covers the file in full. ✅ = file read in full so far
> (verified against the real source); · = pending read. Paths under `src/main/java/com/invoicestudio/`
> are shortened to their tail. 305 tracked files; inventory counts: **272 application
> files + 33 docs/config/scratch files** (counts reconciled in the final audit).

## Build, config & environment

| File | Type | Purpose | Depends on | Used by | Ch | Read |
|---|---|---|---|---|---|---|
...
| File | Type | Purpose | Used by | Ch | Read |
|---|---|---|---|---|---|
...
## Tests (108 files — every one shown in full in its chapter)

| Suite / harness | Tests | Ch |
|---|---|---|
