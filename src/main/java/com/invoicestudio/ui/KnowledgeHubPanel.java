 ...
 *   <li><b>Multi-Level Tree</b>: Expandable/collapsible hierarchy with auto-expanding search filter.</li>
 *   <li><b>Full Monitor Height</b>: Takes 100% of available screen height with independent scrollbars.</li>
 *   <li><b>Persistence</b>: Stored automatically in {@code %APPDATA%\InvoiceStudio\knowledge-hub.json}.</li>
 * </ul>
 */
public class KnowledgeHubPanel extends VBox {
    ...
    private Node buildHero() {
        ...
        Label subtitle = new Label("Documentation, hardware guides, and reference material formatted in Markdown. Add and edit topics at any level.");
    ...
    private Node buildBody() {
        // ── 1. Left Sidebar ──────────────────────────────────────────
        searchField.setPromptText("🔍  Search topics or content…");
        ...
        HBox split = new HBox(12, sidebar, contentArea);
    ...
    private void buildViewMode() {
        ...
        VBox titleGroup = new VBox(3, breadcrumbLbl, contentTitle, contentSubtitle);
    ...
    private void buildEditMode() {
        ...
        Label pathLbl = new Label("Topic Category Path (use / to nest levels, e.g. TSC / TSPL Language):");
        ...
        Label hintLbl = new Label("Tip: Markdown tables, code syntax, and > [!NOTE] callouts are formatted automatically.");
        ...
        editModeContainer.getChildren().setAll(
                ...
                new VBox(4, titleLbl, titleField),
    ...
    private void saveCurrentEdit() {
        ...
        if (isCreatingNew || currentArticle == null) {
            ...
            toSave = new KnowledgeArticle(newId, path, title, subtitle, markdown, System.currentTimeMillis());
        } else {
            toSave = currentArticle.withUpdates(path, title, subtitle, markdown);
        }
    ...
    private void rebuildTree() {
        ...
        // Render each top level category under root (e.g. "TSC")
    ...
    private void renderCategoryNode(KnowledgeRepository.CategoryNode node, String query, int depth, int[] count) {
        ...
        if (searching) {
            node.setExpanded(true); // Auto-expand matching branches while searching
        }

        treeBox.getChildren().add(buildGroupRow(node, depth));
    ...
    private Node buildGroupRow(KnowledgeRepository.CategoryNode group, int depth) {
        Label arrow = new Label(group.isExpanded() ? "▾" : "▸");
